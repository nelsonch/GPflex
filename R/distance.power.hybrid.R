...
#'
#' The function computes the powered distances of any two points in the training set without multiplied by the sensitivity parameters, i.e.,  
#' (x_ik - x_jk)^p_k for any i < j and k < d. 
#' 
...
#' @return dup returns a n*(n-1)/2 by d matrix contains the n*(n-1)/2 combinations of (x_ik - x_jk)^p_k for any i < j and k < d.   

dist.R.power.hybrid <- function(x, p){
  n<-nrow(x)
  d<-mat.or.vec(n,n)
  inds=n*(n-1)/2
  ...
  ttm<-list(d=d, odut=odut, dup=dup)
  return(ttm)
}
